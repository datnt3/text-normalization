        ...
        for i in range(len(parts)):
            ...
            if prefix_match:
                ...
                if len(splitted_value_part) == 2:
                    ...
                    if prefix == "ngày ":
                        spoken_words.append(
                            f"{prefix}{num_to_words(day)} tháng {num_to_words(month)}"
                        )
                    else:
                        spoken_words.append(
                            f"{prefix}ngày {num_to_words(day)} tháng {num_to_words(month)}"
                        )
                    ...
                    if prefix == "ngày ":
                        spoken_words.append(
                            f"{prefix}{num_to_words(day)} tháng {num_to_words(month)} năm {num_to_words(year)}"
                        )
                    else:
                        spoken_words.append(
                            f"{prefix}ngày {num_to_words(day)} tháng {num_to_words(month)} năm {num_to_words(year)}"
                        )
            else:
                value_part = parts[i]
                splitted_value_part = re.split(r"[\.\/]", value_part)
                if len(splitted_value_part) == 2:
                    ...
                    month = splitted_value_part[1]
                    spoken_words.append(
                        f"ngày {num_to_words(day)} tháng {num_to_words(month)}"
                    )
                elif len(splitted_value_part) == 3:
                    day = splitted_value_part[0]
                    month = splitted_value_part[1]
                    year = splitted_value_part[2]
                    spoken_words.append(
                        f"ngày {num_to_words(day)} tháng {num_to_words(month)} năm {num_to_words(year)}"
                    )
    ...
    def mymy_convert(self, value: str) -> str:
        ...
        for i in range(len(parts)):
            ...
            if prefix_match:
                ...
                spoken_words.append(
                    f"{prefix}{num_to_words(month)} năm {num_to_words(year)}"
                )
            else:
                value_part = parts[i]
                month, year = re.split(r"[\.\/]", value_part)
                spoken_words.append(
                    f"tháng {num_to_words(month)} năm {num_to_words(year)}"
                )
